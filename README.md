...
### Tile-Matching Storage Puzzle

You are given a simplified tile-matching puzzle game. The input is an array of objects, where each object represents a tile in the game:

```js
const tiles = [
  { "t1:banana": ["t2:apple", "t3:banana", "t4:orange"] },
  { "t5:apple": ["t4:orange"] },
  { "t6:strawberry": [] },
  { "t7:lemon": ["t2:apple", "t3:banana"] },
  ...
];
```
...
1. **Tile representation**
   ...
   * Each tile key has the format `"tileId:tileType"`.
   * For example, `"t1:banana"` is a tile with id `t1` and type `banana`.

2. **Covering relationship**

   * The value array of a tile lists the tiles that are **covering** it in a 3D grid-like structure.
   * Example: `"t1:banana": ["t2:apple", "t3:banana"]` means `t2` and `t3` must be removed before `t1` can be moved.
   * A tile is **movable** if **no tiles are covering it** (i.e., its covering array is empty).

3. **Storage rules**

   * You have a storage area that can hold up to **6 tiles**.
   * Whenever **3 tiles of the same type** are in storage, they are immediately cleared (all 3 removed from storage).
   * If the storage ever fills to 6 tiles and cannot be reduced immediately, the game is **lost**.

4. **Game objective**

   * Move all tiles from the board into storage (where they will eventually clear in groups of 3).
   * Only movable tiles (as defined above) can be placed into storage.
   * Once a tile is moved, it is removed from the board and also removed from the covering lists of any other tiles it was blocking.
   * The game is **won** if all tiles are cleared without ever hitting the losing condition.

Task

* **Write an algorithm** that determines whether the given game state can be solved (i.e., all tiles cleared).
* If solvable, return at least one valid sequence of moves that leads to a win.
* If not solvable, indicate that no winning sequence exists.
...
One possible winning sequence:

* Move `t2:banana` to storage.
* Move `t3:banana` to storage.
* Move `t1:banana` to storage → 3 bananas clear immediately → storage empty → all tiles cleared.

**Important Note**: There could be more than one candidate tiles available at any given time for moving to storage. But, what tiles to choose to move to storage matters since moving a tile to storage exposes more tiles that can be moved. Thus choosing right tile to move to storage mean better chances of finding a match in storage.
